...
const BRAND = {
  navy: "#1C2D6E", navyDk: "#131f50", red: "#C0392B",
  gold: "#E8A820", goldLt: "#F4D03F", cream: "#F5EFE0", white: "#FFFFFF"
};

const THEMES = {
  light: {
    bg: "#F5EFE0", surface: "#FFFFFF", surfaceAlt: "#FAF7F0",
    border: "#E2D8C0", borderStrong: "#C8B890", text: "#1C1A14",
    textMuted: "#6B5F48", textFaint: "#9E9280", chipBg: "#F0EBE0",
    statBg: "rgba(255,255,255,0.13)", inputBg: "#FAF7F0",
    ...
    filterBg: "#FFFFFF", filterBorder: "#E2D8C0", cardBg: "#FFFFFF",
    cardBorder: "#E2D8C0", bannerBg: `linear-gradient(90deg, ${BRAND.navyDk}, ${BRAND.navy})`,
    mapFooterBg: "#FAF7F0", footerBg: BRAND.navyDk
  },
  dark: {
    bg: "#0F1117", surface: "#1A1D26", surfaceAlt: "#21242F",
    border: "#2E3245", borderStrong: "#404666", text: "#EDE8DC",
    textMuted: "#9A94A8", textFaint: "#605A6E", chipBg: "#21242F",
    statBg: "rgba(255,255,255,0.07)", inputBg: "#21242F",
    ...
    filterBg: "#1A1D26", filterBorder: "#2E3245", cardBg: "#1A1D26",
    cardBorder: "#2E3245", bannerBg: `linear-gradient(90deg, #090C18, #101428)`,
    mapFooterBg: "#21242F", footerBg: "#090C18"
  }
};
...
const Icon = ({ type, active, color }) => {
  const icons = {
    heart: <svg width="17" height="17" viewBox="0 0 24 24" fill={active ? BRAND.red : "none"} stroke={active ? BRAND.red : "#999"} strokeWidth="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>,
    check: <svg width="17" height="17" viewBox="0 0 24 24" fill={active ? "#27ae60" : "none"} stroke={active ? "#27ae60" : "#999"} strokeWidth="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>,
    ...
    ig: <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={color || "currentColor"} strokeWidth="2"><rect x="2" y="2" width="20" height="20" rx="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/></svg>
  };
...
const Logo = ({ size = 80 }) => (
  <img src={logoImg} alt="Logo" width={size} height={size} className="logo-img"
    style={{ width: size, height: size, boxShadow: `0 0 0 4px ${BRAND.gold}55, 0 0 0 8px rgba(255,255,255,0.1), 0 8px 24px rgba(0,0,0,0.2)` }} />
);
...
const Card = ({ b, visited, favorites, onToggleVisit, onToggleFavorite, expandedId, onToggleExpand, imgErr, onImgError, T, dark }) => {
  ...
  return (
    <article className="card" style={{ background: T.cardBg, border: `2px solid ${borderColor}`, boxShadow: `0 2px 10px rgba(0,0,0,${dark ? "0.3" : "0.07"})` }}>
      ...
      <div className="card-image" style={{ background: rc + "18" }} onClick={handlers.expand}>
        {!imgErr.has(b.id) && b.photo ? (
          <img className="bi card-img" src={b.photo} alt={b.dish} onError={handlers.imgError} loading="lazy" />
        ) : (
      ...
      <div className="card-content">
        <div className="card-header">
          ...
          <div className="card-actions">
            <button onClick={handlers.fav} className="action-button" title={isF ? "Remover favorito" : "Favoritar"}><HeartIcon f={isF} /></button>
            <button onClick={handlers.visit} className="action-button" title={isV ? "Desmarcar visitado" : "Marcar visitado"}><CheckCircle d={isV} /></button>
          </div>
        </div>

        <div className="info-bar" style={{ background: T.glassBg, border: `1px solid ${T.glassBorder}` }}>
          <div className="team-visit-status">
            {teamVisited ? <><CheckCircle d={true} /><span className="team-visit-text" style={{ color: "#16A085" }}>Time visitou</span></> : <span className="team-waiting-text" style={{ color: T.textFaint }}>⏳ Aguardando</span>}
          </div>
          {b.rating !== undefined && b.rating !== null && (
            <div className="rating-badge" style={{ background: BRAND.navy }}><span className="rating-star" style={{ color: BRAND.goldLt }}>⭐</span><span className="rating-value" style={{ color: "#fff" }}>{b.rating.toFixed(1)}</span></div>
          )}
        ...
        <p className="description-text" style={{ color: T.textMuted }}>{isE ? b.desc : b.desc.slice(0, 88) + (b.desc.length > 88 ? "…" : "")}</p>

        {isE && <div className="address-text" style={{ color: T.textFaint }}><span>📍</span><span>{b.address} – {b.neighborhood}, BH</span></div>}

        <button onClick={handlers.expand} className="expand-btn" style={{ color: rc }}>{isE ? "Menos ↑" : "Mais ↓"}</button>
      </div>
    </article>
  );
};
...
export default function App() {
  ...
  // Dados processados
  const barsWithNormalized = useMemo(() => BARS.map(bar => ({
    ...bar,
  ...
  const filteredAndSorted = useMemo(() => {
    let result = barsWithNormalized.filter(b => {
      ...
      if (filters.search) {
        ...
        if (!b.normalizedName.includes(q) && !b.normalizedDish.includes(q) && !b.normalizedNeighborhood.includes(q)) return false;
      }
      return true;
    });
  ...
  const handleImgError = useCallback((id) => setImgErr(prev => new Set([...prev, id])), []);
  const handleFilterChange = useCallback((key, value) => setFilters(prev => ({ ...prev, [key]: value })), []);
  const clearFilters = useCallback(() => setFilters({ onlyVisited: false, onlyFavorites: false, region: "Todas", search: "", onlyTeam: false }) || setSortBy("none"), []);
  ...
  const stats = useMemo(() => [{ v: BARS.length, l: "Bares" }, { v: visited.size, l: "Eu visitei" }, { v: favorites.size, l: "Favoritos" }], [visited.size, favorites.size]);

  // Componentes comuns
  const FilterButton = ({ active, onClick, icon, label, activeColor, activeBg, activeText }) => (
    ...
    <div className="app-container" style={{ background: T.bg }}>
      ...
      <header style={{ background: T.headerBg }}>
        <div className="instagram-bar">
          ...
          <a href="https://www.instagram.com/ParticipantesdiButeco" target="_blank" style={{ color: BRAND.goldLt, fontWeight: 700 }}>@ParticipantesdiButeco</a>
        ...
        <div className="header-content">
          <div style={{ display: "flex", alignItems: "center", gap: "2rem", flexWrap: "wrap" }}>
            ...
            <div className="header-stats">
              {stats.map(s => <div key={s.l} className="stat-card" style={{ background: T.statBg }}><div className="stat-value">{s.v}</div><div className="stat-label">{s.l}</div></div>)}
              <button className="dm-toggle" onClick={() => setDark(p => !p)} style={{
                ...
                justifyContent: "center", color: dark ? BRAND.goldLt : "#fff", backdropFilter: "blur(8px)"
          ...
          <div className="tabs-container">
            {[{ k: "bares", l: "🍺  Bares & Pratos" }, { k: "mapa", l: "🗺️  Mapa Interativo" }].map(t => (
              <button key={t.k} onClick={() => setTab(t.k)} className="tab-button" style={{
        ...
        <>
          ...
          <div className="sticky-filters" style={{ background: T.filterBg, borderBottom: `1px solid ${T.filterBorder}`, boxShadow: `0 2px 12px rgba(0,0,0,${dark ? "0.4" : "0.07"})` }}>
            <div className="sticky-filters-inner">
              <div className="search-wrapper">
                ...
                <input type="text" placeholder="Buscar (ex: coracao encontra coração)..." value={filters.search}
                  onChange={e => handleFilterChange("search", e.target.value)} className="search-input"
                  style={{ border: `1.5px solid ${T.border}`, background: T.inputBg, color: T.text }} />
              </div>

              <div className="filters-group">
                <select value={filters.region} onChange={e => handleFilterChange("region", e.target.value)} className="select-input" style={{
                ...
                }}>{REGIONS.map(r => <option key={r}>{r === "Todas" ? "📌 Regiões" : r}</option>)}</select>

                <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="select-input" style={{
                ...
                }}><option value="none">📋 Ordenar</option><option value="rating">⭐ Maior nota</option><option value="beerTemp">🌡️ Mais gelada</option></select>
              </div>

              <div className="filters-group">
                <FilterButton active={filters.onlyTeam} onClick={() => handleFilterChange("onlyTeam", !filters.onlyTeam)} icon={<CheckCircle d={filters.onlyTeam} />} label="Time visitou" activeColor="#27ae60" activeBg={dark ? "#0d2c1a" : "#eafaf1"} activeText="#1a5c30" />
                <FilterButton active={filters.onlyVisited} onClick={() => handleFilterChange("onlyVisited", !filters.onlyVisited)} icon={<CheckCircle d={filters.onlyVisited} />} label="Eu visitei" activeColor="#27ae60" activeBg={dark ? "#0d2c1a" : "#eafaf1"} activeText="#1a5c30" />
                <FilterButton active={filters.onlyFavorites} onClick={() => handleFilterChange("onlyFavorites", !filters.onlyFavorites)} icon={<HeartIcon f={filters.onlyFavorites} />} label="Favoritos" activeColor={BRAND.red} activeBg={dark ? "#2c0d0d" : "#fdf0ed"} activeText="#c0392b" />
              </div>

              {hasActiveFilters && <button onClick={clearFilters} className="filter-pill" style={{ border: `1px solid ${T.border}`, background: T.surface, color: T.textMuted }}>✕ Limpar</button>}
              <div className="filter-counter" style={{ color: T.textFaint }}>{filteredAndSorted.length} / {BARS.length}</div>
            </div>
          </div>
          ...
          <div className="max-width-1200 px-15 mt-2">
            <div onClick={() => setTab("mapa")} className="map-cta" style={{ background: T.bannerBg, boxShadow: `0 2px 14px rgba(28,45,110,${dark ? "0.5" : "0.2"})`, border: `1px solid ${BRAND.navy}55` }}>
              <Logo size={38} />
              <div><div className="cta-title" style={{ color: "#fff" }}>Ver Mapa Interativo dos Bares</div><div className="cta-subtitle" style={{ color: BRAND.goldLt }}>Todos os bares marcados no mapa personalizado · Curadoria @ParticipantesdiButeco</div></div>
          ...
          <main className="max-width-1200 px-15" style={{ margin: "0 auto", padding: "1.8rem 1.5rem 3.5rem" }}>
            {filteredAndSorted.length === 0 ? (
              <div className="empty-state"><div className="empty-icon">🔍</div><p style={{ color: T.textMuted }}>Nenhum bar encontrado com esses filtros.</p><button onClick={clearFilters} className="empty-button" style={{ background: BRAND.navy, color: "#fff" }}>Limpar filtros</button></div>
            ) : sortBy !== "none" ? (
              <div className="bars-grid">{filteredAndSorted.map(b => <Card key={b.id} b={b} visited={visited} favorites={favorites} onToggleVisit={handleToggleVisit} onToggleFavorite={handleToggleFavorite} expandedId={expandedId} onToggleExpand={handleToggleExpand} imgErr={imgErr} onImgError={handleImgError} T={T} dark={dark} />)}</div>
            ) : (
              Object.keys(grouped).sort().map(region => (
                <section key={region} className="section-margin">
                  ...
                  <div className="bars-grid">{grouped[region].map(b => <Card key={b.id} b={b} visited={visited} favorites={favorites} onToggleVisit={handleToggleVisit} onToggleFavorite={handleToggleFavorite} expandedId={expandedId} onToggleExpand={handleToggleExpand} imgErr={imgErr} onImgError={handleImgError} T={T} dark={dark} />)}</div>
                </section>
              ))
            )}
          </main>
      ...
      <footer style={{ background: T.footerBg, padding: "2.5rem 1.5rem", textAlign: "center" }}>
        ...
        <div className="footer-credit"><IgIcon color={BRAND.goldLt} /><span style={{ fontFamily: "sans-serif", fontSize: "0.85rem", color: "#aaa" }}>Curadoria: <a href="https://www.instagram.com/ParticipantesdiButeco" target="_blank" style={{ color: BRAND.goldLt, fontWeight: 700 }}>@ParticipantesdiButeco</a></span></div>
        <p className="footer-text">Comida di Buteco 2026 · 26ª Edição · Belo Horizonte · 10 abr – 10 mai · Petiscos a R$ 40</p>
        <button onClick={() => setDark(p => !p)} className="footer-theme-toggle" style={{ color: dark ? BRAND.goldLt : "#ccc" }}>{dark ? <SunIcon /> : <MoonIcon />}{dark ? "Modo Claro" : "Modo Escuro"}</button>
      </footer>
    </div>
  );
}
